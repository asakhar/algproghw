...
def encode(plain_text, a, b):
    m = 26
    if not arecoprime(a, m):
        raise ValueError('Error: a and m must be coprime.')
    ret = ''
    s = plain_text.replace(' ', '').lower().replace(',', '').replace('.', '')
    c = 1
    for i in s:
        if i.isnumeric():
            ret += i
        else:
            ret += chr((a*(ord(i)-97)+b)%26+97)
        if c%5==0:
            ret += ' '
        c += 1
    if ret[-1]==' ':
...
def decode(ciphered_text, a, b):
    ...
    if not arecoprime(a, m):
        raise ValueError('Error: a and m must be coprime.')
    ret = ''
    s = ciphered_text.replace(' ', '').lower().replace(',', '').replace('.', '')
    for i in s:
        if i.isnumeric():
            ret += i
        else:
            ret += chr(amod(a, m)*(ord(i)-97-b)%m+97)
    return ret 
